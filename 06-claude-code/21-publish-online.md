...
3. Точка входа — файл `index.html` в корне проекта или в папке `docs/`.
...
Точка входа — index.html в корне.
```

Claude проверит что репозиторий подключён, что есть нужные файлы, и подскажет что делать дальше. Если в системе установлен `gh` (GitHub CLI) — Claude включит Pages сам через одну команду. Если нет — даст пошаговую инструкцию для интерфейса GitHub из Способа 1.

**Если Claude спрашивает про ветку или папку:**

- Ветка обычно — **main**

- Папка — **/** (корень) если `index.html` в корне, или **/docs** если в папке docs
